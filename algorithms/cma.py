...
class CMAMaster(algorithms.abc_algorithm.BaseESMaster):
    ...
    def __init__(self,
        ...
        super(CMAMaster, self).__init__(
            ...
            start_iter=start_iter if start_iter is not None else 0
        )
        if load_model_path:
            self._solution.load(load_model_path)
            self._logger.info('Load model with {}.'.format(load_model_path))
...
class CMAWorker(algorithms.abc_algorithm.BaseESWorker):
    ...
    def __init__(self, logger, load_model_path=None):
