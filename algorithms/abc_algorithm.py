...
class BaseESMaster(abc.ABC):
    ...
    def __init__(self,
        ...
        self._start_iter = start_iter if start_iter is not None else 0
    ...
    def train(self):
        ...
        for iter_cnt in range(self._start_iter, self._max_iter):

            # Training.
            start_time = time.time()
            scores = self._train_once()
            time_cost = time.time() - start_time
            self._logger.info('1-step training time: {:.1f}s'.format(time_cost))
            misc.utility.log_scores(
